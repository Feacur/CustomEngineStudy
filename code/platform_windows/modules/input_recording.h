...
static LPCWSTR const state_file_name = L"StateSnapshot.blob";
static LPCWSTR const input_file_name = L"InputRecord.blob";

void save_state(Memory_Chunk * permanent_memory) {
	HANDLE file = CreateFileW(state_file_name, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
...
void load_state(Memory_Chunk * permanent_memory) {
	HANDLE file = CreateFileW(state_file_name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
	if (file != INVALID_HANDLE_VALUE) {
		DWORD bytes_read;
...
void process_input_recording(Memory_Chunk *permanent_memory, int32 input) {
	...
	if (input_recording_state == Input_Recording_State::Record) {
		if (previous_state != Input_Recording_State::Record) {
			...
			input_file = CreateFileW(input_file_name, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
		}
	...
	if (input_recording_state == Input_Recording_State::Play) {
		if (previous_state != Input_Recording_State::Play) {
			...
			input_file = CreateFileW(input_file_name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
		}
