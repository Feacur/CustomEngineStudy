...
static int Lua_Script_index(lua_State * L) {
	...
	if (strcmp(id, "update") == 0) {
		cstring value = custom::strings_storage.get_value(object->get_fast()->update_string_id);
...
static int Lua_Script_newindex(lua_State * L) {
	...
	if (strcmp(id, "update") == 0) {
		...
		object->get_fast()->update_string_id = custom::strings_storage.get_or_add_id(value, (u32)strlen(value));
		return 0;
	}
